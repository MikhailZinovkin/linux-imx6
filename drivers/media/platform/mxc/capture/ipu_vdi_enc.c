 ...
 * @return  status
 */
static int vdi_enc_eba_update(void *private, struct ipu_soc *ipu, dma_addr_t eba, int *buffer_num)
{
...
static int vdi_enc_disabling_tasks(void *private)
{
	...
	ipu_uninit_channel(cam->ipu, MEM_VDI_PRP_VF_MEM_P, NULL);
	ipu_uninit_channel(cam->ipu, MEM_VDI_PRP_VF_MEM, NULL);
	ipu_uninit_channel(cam->ipu, MEM_VDI_PRP_VF_MEM_N, NULL);
