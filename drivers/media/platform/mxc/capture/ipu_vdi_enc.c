 ...
 * @file ipu_vdi_enc.c
 *
 * @brief CSI->VDI->MEM Use case for video capture
 *
 * @ingroup IPU
 */
...
static int vdi_enc_setup(cam_data *cam)
{
	...
	ipu_csi_get_window_size(cam->ipu, &params.csi_vdi_mem.in_width,
				&params.csi_vdi_mem.in_height, cam->csi);

	params.csi_vdi_mem.in_pixel_fmt = IPU_PIX_FMT_UYVY;
	params.csi_vdi_mem.out_width = cam->v2f.fmt.pix.width;
	params.csi_vdi_mem.out_height = cam->v2f.fmt.pix.height;
	params.csi_vdi_mem.csi = cam->csi;
	params.csi_vdi_mem.field_fmt = V4L2_FIELD_INTERLACED_BT;
	params.csi_vdi_mem.motion_sel = HIGH_MOTION;
	...
	err = ipu_enable_channel(cam->ipu, CSI_VDI_MEM);
	if (err < 0) {
		printk(KERN_ERR "ipu_enable_channel CSI_VDI_MEM\n");
		return err;
	}
 ...
 * @return  status
 */
static int vdi_enc_eba_update(struct ipu_soc *ipu, 
			      dma_addr_t eba, int *buffer_num)
{
	int err = 0;

	pr_debug("eba %x\n", eba);
	err = ipu_update_channel_buffer(ipu, CSI_VDI_MEM, IPU_OUTPUT_BUFFER,
					*buffer_num, eba);
	if (err != 0) {
		ipu_clear_buffer_ready(ipu, CSI_VDI_MEM, IPU_OUTPUT_BUFFER,
				       *buffer_num);
	...
	ipu_select_buffer(ipu, CSI_VDI_MEM, IPU_OUTPUT_BUFFER, *buffer_num);
...
static int vdi_enc_enabling_tasks(void *private)
{
	...
	ipu_clear_irq(cam->ipu, IPU_IRQ_VDIC_OUT_EOF);
	err = ipu_request_irq(cam->ipu, IPU_IRQ_VDIC_OUT_EOF,
...
MODULE_DESCRIPTION("CSI VDI ENC Driver");
MODULE_LICENSE("GPL");
